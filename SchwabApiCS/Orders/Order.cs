...
namespace SchwabApiCS.Orders
{
    ...
    public class Order
    {
        ...
        public Order(OrderType orderType, OrderStrategyTypes orderStrategyTypes, Session session, Duration duration, decimal? price = null)
        {
            ...
            orderStrategyType = orderStrategyTypes.ToString();
            this.session = session.ToString();
            this.duration = duration.ToString();
            if (orderType == OrderType.STOP)
                stopPrice = PriceAdjust(price);
            else
        ...
        {
            return GetEnum<OrderType>(orderType);
        }

        public static Status GetOrderStatus(string orderStatus)
        {
            return GetEnum<Status>(orderStatus);
        }
        ...
        public static AssetType GetAssetType(string assetType)
        {
            ...
            return GetEnum<AssetType>(assetType);
        }
        ...
        public static Duration GetDuration(string duration)
        {
            ...
            return GetEnum<Duration>(duration);
        }

        public static Session GetSession(string session)
        {
            return GetEnum<Session>(session);
        }


        public static OrderStrategyTypes GetOrderStrategyType(string orderStrategyType)
        {
            return GetEnum<OrderStrategyTypes>(orderStrategyType);
        }
            ...
            private void CalculateInstruction(AssetType assetType, Position position, decimal quantity)
            {
                if (assetType == AssetType.OPTION)
                {
                    if (position == Position.TO_OPEN)
                        instruction = (quantity > 0 ? Instruction.BUY_TO_OPEN : Instruction.SELL_TO_OPEN).ToString();
                    else // TO_CLOSE
                        instruction = (quantity > 0 ? Instruction.BUY_TO_CLOSE : Instruction.SELL_TO_CLOSE).ToString();
                }
                else
                {
                    if (position == Position.TO_OPEN)
                        instruction = (quantity > 0 ? Instruction.BUY : Instruction.SELL_SHORT).ToString();
                    else // TO_CLOSE
                        instruction = (quantity > 0 ? Instruction.BUY_TO_COVER : Instruction.SELL).ToString();
                }
            }
        ...
        public static Order OrderLimit(string instruction, string symbol, AssetType assetType, Duration duration,
                                  Session session, decimal quantity, decimal price)
        {
        ...
        public static Order OrderMarket(string instruction, string symbol, AssetType assetType, Duration duration,
                                  Session session, decimal quantity)
        {
        ...
        private static Order CreateOrder(string instruction, OrderType orderType, string symbol, AssetType assetType, Duration duration,
                                          Session session, decimal quantity, decimal? price = null)
        {
        ...
        public static Order OrderStopLoss(string symbol, AssetType assetType, Duration duration,
                                          Session session, decimal quantity, decimal stopPrice)
        {
            var order = new Order()
            {
